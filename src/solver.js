...
function initWeights(totalSize, rounds) {
	const weights = _.range(totalSize).map(() => _.range(totalSize).fill(0));
	rounds.forEach((round) => updateWeights(round, weights));
	return weights;
}
...
function solver(
	...
	//현재 가중치 기준 라운드 점수 반환
	function getScoredRound(round, weights) {
	...
	function generateValidRound(groupNum) {
		function generateRound() {
			const people = _.shuffle([...peopleArr]);
			const round = _.range(groupNum).map((i) =>
				_.range(peoplePerGroup).map((j) => people[i * peoplePerGroup + j])
			);
			return round;
		}
		function validateRound(round) {
			return round.every((group) => group.filter((e) => e === null).length < 2);
		}
		while (true) {
		...
		candidates.forEach((candidate) => {
			...
			for (let i = 0; i < peoplePerGroup; i++) {
				for (let j = peoplePerGroup; j < totalSize; j++) {
					mutations.push(
						getScoredRound(getSwappedRound(sorted, i, j), weights)
					);
				}
			}
	...
	function getSwappedRound(groups, i, j) {
	...
	let roundScore = 0;
	const weights = initWeights(totalPeopleNum, rounds);
	let topOptions = _.range(5).map(() =>
		getScoredRound(generateValidRound(groupNum), weights)
	);
